...
class Metabox
{
    ...
    function __construct($title, $postType, $fields)
    {
        ...
        $this->fields = $fields;
        $this->nonceKey = RECIPE_UNIT_CONVERSION_KEY . $this->id . 'Nonce';
    ...
    public function render($post){
        ...
        if(is_array($this->fields) && !empty($this->fields)){
            foreach($this->fields as $field){
                $fieldRow = '<div class="themeMetaboxRow">';
                $fieldId = str_replace('-', '_', Strings::lower(Strings::webalize($field['label']))); // '_' .
                ...
                switch($field['type']){
                    ...
                    case 'tel':
                    case 'email':
                        $fieldInput = '<input id="'. $fieldId .'" name="'. $fieldId .'" type="' . $field['type'] . '" value="'. $fieldValue .'" />';
                        break;
                    case 'select':
                        $fieldInput = '<select id="'. $fieldId .'" name="'. $fieldId .'">';
                        if(!empty($fieldOptions) && is_array($fieldOptions)){
                            foreach($fieldOptions as $key => $option){
                                if(Strings::webalize($option) == $fieldValue){ $selected = 'selected'; } else { $selected = ''; }
                                $fieldInput .= '<option value="' . Strings::webalize($option) . '" '. $selected .'>'. $option .'</option>';
                            }
                        }
                        $fieldInput .= '</select>';
                        break;
                    case 'textarea':
                        $fieldInput = '<textarea id="'. $fieldId .'" name="'. $fieldId .'">' . $fieldValue . '</textarea>';
                        break;
                    case 'checkbox':
                        if(true == $fieldValue){ $checked = 'checked'; } else { $checked = ''; }
                        $fieldInput = '<input id="'. $fieldId .'" name="'. $fieldId .'" value="true" type="'. $field['type'] .'" '. $checked .' />';
                        break;
                    case 'radio':
